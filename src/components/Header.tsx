...
export default function Header({ currentRoute, onNavigate, currentUser, onLogout }: HeaderProps) {
  ...
  const navItems = [
    { name: 'Home', route: { name: 'home' } as AppRoute },
    { name: 'All Tiles', route: { name: 'all-tiles' } as AppRoute },
    { name: 'My Profile', route: { name: 'my-profile' } as AppRoute, private: true },
  ];
    ...
    <header className="sticky top-0 z-40 w-full border-b border-neutral-200 bg-white/80 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navItems.map((item) => {
            // Only hide private items if no current user (or show and let route guard handle it)
        ...
        <div className="hidden md:flex items-center gap-4">
          {currentUser ? (
            <div className="flex items-center gap-4">
              <button
                onClick={() => handleNavClick({ name: 'my-profile' })}
            ...
            {navItems.map((item) => (
              <button
                ...
                className={`text-left text-sm font-medium py-1.5 transition-colors ${
                  isActive(item.route.name) ? 'text-neutral-900 font-bold' : 'text-neutral-500'
                }`}
              >
                {item.name}
              </button>
